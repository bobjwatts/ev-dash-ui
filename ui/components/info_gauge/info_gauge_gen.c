 ...
 *
 * Panels:
 *   0 Pack voltage   — full blue ring
 *   1 Cell balance   — full green ring
 *   2 Efficiency     — full teal ring
 *   3 Charging       — green ring filled to SOC% (only when sys_state == SYS_CHARGE)
 */
...
static void info_arc_soc_cb(lv_observer_t * observer, lv_subject_t * subject);
...
static void info_gauge_set_arc_full(lv_color_t color, int value_pct)
{
...
static void info_gauge_refresh(void)
{
    ...
    switch(panel) {
        case INFO_PANEL_PACK_VOLTAGE:
            info_gauge_set_arc_full(COLOR_BLUE, 100);
        ...
        case INFO_PANEL_CELL_BALANCE:
            info_gauge_set_arc_full(COLOR_GREEN, 100);
        ...
        case INFO_PANEL_EFFICIENCY:
            info_gauge_set_arc_full(INFO_COLOR_TEAL, 100);
        ...
        case INFO_PANEL_CHARGING:
            info_gauge_set_arc_full(COLOR_GREEN, lv_subject_get_int(&state_of_charge_pct));
            lv_snprintf(buf, sizeof(buf), "%d%%", lv_subject_get_int(&state_of_charge_pct));
            lv_label_set_text(s_ui.main_value, buf);
...
{
    LV_UNUSED(observer);
    if(lv_subject_get_int(&info_panel) != INFO_PANEL_CHARGING) return;
    int soc = lv_subject_get_int(subject);
    info_gauge_set_arc_full(COLOR_GREEN, soc);
    char buf[8];
    lv_snprintf(buf, sizeof(buf), "%d%%", soc);
    lv_label_set_text(s_ui.main_value, buf);
}

static void info_gauge_bind_observers(void)
{
    lv_subject_add_observer(&info_panel, info_observer_cb, NULL);
    lv_subject_add_observer(&sys_state, info_observer_cb, NULL);
    lv_subject_add_observer(&battery_voltage_v, info_observer_cb, NULL);
    lv_subject_add_observer(&cell_balance_mv, info_observer_cb, NULL);
    lv_subject_add_observer(&efficiency_wh_per_km, info_observer_cb, NULL);
    lv_subject_add_observer(&energy_kwh_remaining, info_observer_cb, NULL);
    lv_subject_add_observer(&batt_temp_c, info_observer_cb, NULL);
    lv_subject_add_observer(&inverter_temp_c, info_observer_cb, NULL);
    lv_subject_add_observer(&aux_voltage_v, info_observer_cb, NULL);
    lv_subject_add_observer(&charge_amps_a, info_observer_cb, NULL);
    lv_subject_add_observer_obj(&state_of_charge_pct, info_arc_soc_cb, s_ui.main_value, NULL);
}
...
lv_obj_t * info_gauge_create(lv_obj_t * parent)
{
    ...
    /* Theme arc — full ring on panels 0–2; SOC-proportional on charging */
    ...
    lv_obj_set_style_arc_color(s_ui.theme_arc, COLOR_GAUGE_TRACK, LV_PART_MAIN);
    ...
    lv_arc_set_value(s_ui.theme_arc, 100);
