...
static esp_err_t lvgl_port_panel_swap_xy_if_needed(esp_lcd_panel_handle_t panel, lv_display_rotation_t rotation,
        bool cfg_swap_xy)
{
    bool swap;
    ...
    return esp_lcd_panel_swap_xy(panel, swap);
}
...
static void lvgl_port_disp_rotation_update(lvgl_port_display_ctx_t *disp_ctx)
{
    ...
    lv_display_rotation_t rotation = lv_display_get_rotation(disp_ctx->disp_drv);
    /* Solve rotation screen and touch */
    switch (rotation) {
    case LV_DISPLAY_ROTATION_0:
        /* Rotate LCD display */
        lvgl_port_panel_swap_xy_if_needed(control_handle, rotation, disp_ctx->rotation.swap_xy);
